            ...
            <div className="hidden lg:flex flex-col w-54 h-screen bg-green-900 text-white fixed top-0 left-0">
              ...
              <ul className="flex-1 flex flex-col gap-1 w-full overflow-y-auto">
                {items.map((item, index) => {
                  ...
                  const itemHidden = item.lebel == "Profile"
      
                  return (
                    <li key={index}>
                      <Link
                        to={item.path}
                        className={`flex items-center gap-3 p-3 rounded ${isActive ? "bg-green-700" : "hover:bg-green-500/30"}`}
                      >
                        <Icon size={24} />
                        <span className={`text-lg ${itemHidden ? "hidden" : null}`} >{item.label}</span>
                        {badgeValue && (
